...
export const ContactForm = () => {
  ...
  const validateForm = (formData: FormData) => {
    ...
    if (!emailRegex.test(email)) {
      newErrors.email = "⚠️ รูปแบบอีเมลไม่ถูกต้องค่ะ";
    }

    const tel = formData.get("tel") as string;
    const telRegex = /^0[0-9]{9}$/;
    if (!telRegex.test(tel.replace(/[-\s]/g, ""))) {
      newErrors.tel = "⚠️ ต้องขึ้นต้นด้วย 0 และมี 10 หลักค่ะ";
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      alert("ได้รับข้อมูลแล้วค่ะ ทีม Biz Lab จะติดต่อกลับหาคุณเร็ว ๆ นี้นะคะ");
      form.reset();
    } catch (error) {
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              ...
              <div className="flex flex-col md:flex-row md:items-start gap-2">
                <label className="w-full md:w-[40%] text-left text-[11px] font-black text-slate-700 font-sans uppercase tracking-wider md:pt-1.5">อีเมล*</label>
              ...
              <div className="flex flex-col md:flex-row md:items-start gap-2">
                <label className="w-full md:w-[40%] text-left text-[11px] font-black text-slate-700 font-sans uppercase tracking-wider md:pt-1.5">เบอร์โทรศัพท์*</label>
                <div className="w-full md:w-[60%]">
                  <input
                    required
                    name="tel"
                  ...
                  />
                  {errors.tel && <p className="text-red-500 text-[10px] mt-1">{errors.tel}</p>}
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-2 mt-2">
                ...
                <div className="w-full md:w-[60%]">
                  <button
                    ...
                    type="submit"
                    className="w-full md:w-auto bg-[#0B132B] hover:bg-black text-white text-[11px] font-black py-2.5 px-8 rounded-full transition-all shadow-md active:scale-95 font-sans uppercase tracking-tight disabled:bg-slate-400"
                  >
          ...
          <div className="flex flex-col items-center justify-center relative md:h-full mt-10 md:mt-0">
            ...
            >
              ...
              <span className="text-[14px] font-black tracking-wide drop-shadow-sm">แชทปรึกษาทีมงานผ่าน LINE</span>
            </motion.a>
